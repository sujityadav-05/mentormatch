...
// ✅ Cancel Request (Mentee → Cancel pending request)
export const cancelRequest = async (req, res) => {
  try {
    ...
    if (!connection) {
      return res.status(400).json({ message: "No pending request to this mentor" });
    }

    res.status(200).json({ message: "Request cancelled" });
  } catch (error) {
    console.log("Error in cancelRequest:", error.message);
    res.status(500).json({ message: "Internal Server Error" });
  }
};
...
export const getMentors = async (req, res) => {
  try {
    ...
    const { search, expertise, minRating, page = 1, limit = 10 } = req.query;
    ...
    const mentors = await User.find(filter)
      .select("fullName email profilePic averageRating totalRatings expertise bio")
      .skip(skip)
      .limit(limitNum)
